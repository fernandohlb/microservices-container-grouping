...
source("results_functions.R")
      ...
      if (with_hpa) {
        ...
        destination_folder = "graphics_and_tables_by_time/with_hpa/performance"
      } else {
        origem_folder = "raw_results"
        destination_folder = "graphics_and_tables_by_time/not_hpa/performance"
      }
  ...
  ######################### REQUESTS PER SECOND RESULTS #########################
      figRequests.s.scenario <- ggplot(consolidated_table, aes(x=factor(seconds), y=Requests.s, color = factor(scenario))) +
  ...
  requests_s_summarized_table = consolidate_summary_v2(consolidated_table,consolidated_table$Requests.s,workloads,scenarios)
  write.table(requests_s_summarized_table, table_file, sep=",",row.names=FALSE)
  dev.off()
  
  
  ######################### REPONSE TIME RESULTS #########################
  figResponseTimeScenario <- ggplot(consolidated_table, aes(x=factor(seconds), y=Total.Average.Response.Time, color = factor(scenario))) +
    geom_boxplot(position = position_dodge(width = 0.9)) +
    stat_summary(
      fun = median,
      geom = 'line',
  ...
  response_time_summarized_table = consolidate_summary_v2(consolidated_table,consolidated_table$Total.Average.Response.Time,workloads,scenarios)
  ...
  summarized_table <- consolidated_table %>% group_by(workload,scenario,seconds)%>%
  ...
  figFailures.s.scenario <- ggplot(consolidated_table, aes(x=factor(seconds), y=Failures.s, color = factor(scenario))) +
    geom_boxplot(position = position_dodge(width = 0.9)) +
    stat_summary(
      fun = median,
      geom = 'line',
    ...
    failures_per_second_summarized_table = consolidate_summary_v2(consolidated_table,consolidated_table$Failures.s,workloads,scenarios)
